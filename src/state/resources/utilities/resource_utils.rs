...
pub fn load_texture_byte_or_reuse(main_queue: ExecutionQueueItem, max_tex_res: u32, create_mipmaps: bool, image_bytes: &Vec<u8>, name: &str, path: &str, extension: Option<String>) -> TextureItem
{
    ...
    // ***** if not found -> load *****
    let arc = load_texture_byte(max_tex_res, create_mipmaps, image_bytes, name, path, extension);
...
pub fn load_texture(max_tex_res: u32, create_mipmaps: bool, path: &str, extension: Option<String>) -> anyhow::Result<TextureItem>
{
    let image_bytes = load_binary(path)?;
    let name = file::get_stem(path);

    Ok(load_texture_byte(max_tex_res, create_mipmaps, &image_bytes, name.as_str(), path, extension))
}

pub fn load_texture_byte(max_tex_res: u32, create_mipmaps: bool, image_bytes: &Vec<u8>, name: &str, path: &str, extension: Option<String>) -> TextureItem
{
